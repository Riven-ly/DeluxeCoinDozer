Shader "RVFX/UI/EdgeEffects/EdgeEffect_Additive_Rounded"
{
    Properties
    {
        [Header(Thickness)]
        ...
        // 【新增】圆角半径控制 (0 = 直角, 1 = 全圆)
        _CornerRadius("Corner Radius", Range(0, 1)) = 0.0 
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float2 centered_uv = (i.edge_uv - 0.5) * 2.0;
                    ...
                    // --- 修改开始：支持圆角 ---
                    // 1. 获取基础坐标 (Abs对称)
                    float2 d = abs(centered_uv);
                    // 2. 应用 X 轴厚度缩放 (保留原有逻辑)
                    d.x = pow(d.x, _XThicknessScaleFactor);

                    // 3. 计算圆角逻辑
                    // b 是圆角开始的边界 (1.0 代表边缘)
                    float r = clamp(_CornerRadius, 0.0, 1.0);
                    float b = 1.0 - r; 

                    // 计算超出内部矩形部分的向量 (只在角落处 > 0)
                    float2 d_corner = max(d - b, 0.0);
                    
                    // 核心算法：
                    // 如果在直边区域：min(max(d.x, d.y), b) 生效，d_corner 为 0，结果为 max(x,y)
                    // 如果在角落区域：min 结果锁定在 b，加上 d_corner 的长度 (圆形距离)
                    u = length(d_corner) + min(max(d.x, d.y), b);
                    // --- 修改结束 ---
                #endif
